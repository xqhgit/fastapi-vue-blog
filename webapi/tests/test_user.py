...
def test_login(test_app: TestClient):
    ...
    response = test_app.post(url='/admin/login/access_token/', data=form_data)
...
def test_login_getinfo(test_app: TestClient):
    ...
    response = test_app.get(url='/admin/login/getinfo', headers=headers)
    assert response.status_code == 200


def test_login_getinfo_error(test_app: TestClient):
    response = test_app.get(url='/admin/login/getinfo')
